...
from torch.autograd import Variable #封装grad的tensor
...
class ElementWiseConv2d(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
        ...
        kernel_size = _pair(kernel_size)  #tuple (s,s)
        stride = _pair(stride)
        ...
        self.weight = Parameter(torch.Tensor(out_channels, in_channels // groups, *kernel_size), requires_grad=False)
        if bias:
        ...
        if mask_init == '1s':  #以定（小）值初始化
            self.mask_real.fill_(mask_scale)  
        elif mask_init == 'uniform':  #均匀随机初始化
